...
class KeychainSwift {
  ...
  Specify an access group that will be used to access keychain items. Access groups can be used to share
   keychain items between applications. When access group value is nil all application access groups are being accessed. Access group name is used by all functions: set, get, delete and clear.
...
//    let attributes: [String: Any] = [KeychainSwiftConstants.valueData: value]
  ...
  */
  open func getData(_ key: String, asReference: Bool = false) -> Data? {
    ...
    if lastResultCode == noErr {
      ...
      guard lastResultCode != errSecItemNotFound else {
          return nil
      }
      ...
      guard let existingItem = result as? [String: Any] else { return nil }
      guard let returnValue = existingItem[KeychainSwiftConstants.valueData] as? Data else { return nil }
    return returnValue
  }
...
//  After the first unlock, the data remains accessible until the next restart. This is recommended for items that need to be
//   accessed by background applications. Items with this attribute migrate to a new device when using encrypted backups.
  case accessibleAfterFirstUnlock


//  The data in the keychain item cannot be accessed after a restart until the device has been unlocked once by the user.
//  After the first unlock, the data remains accessible until the next restart. This is recommended for items that need to be
//   accessed by background applications. Items with this attribute do not migrate to a new device. Thus, after restoring from a backup of a different device, these items will not be present.
...
//   Items with this attribute never migrate to a new device. After a backup is restored to a new device, these
//   items are missing. No items can be stored in this class on devices without a passcode. Disabling the device passcode causes all items in this class to be deleted.
